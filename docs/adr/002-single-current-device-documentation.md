...
- Automatically rendered documentation consists only of localized text paragraphs.
- Images, videos, specialized layouts, and behavior belong exclusively to custom documentation components.
- Automatic documentation paragraphs are rendered directly without device-version filtering or type dispatch.
...
Automatically rendered documentation nodes contain their current localized paragraphs directly:
...
Each string is one paragraph, in display order. The automatic content schema has no `type` discriminator because it supports only text.

Custom documentation components are selected outside the automatic content renderer. They own their layout, media, behavior, and any component-specific data contract. Localized alt text, captions, or labels required by a custom component must live in data scoped to that component instead of widening the generic documentation schema.

Version-specific fields such as `vpetVersions`, `changes`, and maps of text by version are not part of either model.
...
- The automatic renderer reflects its actual responsibility: rendering localized paragraphs.
- Media-specific concerns stay close to the custom components that use them.
...
- Reusable media behavior must be implemented explicitly by custom components.
- Custom components that need localized media metadata require their own scoped data contracts.

## Superseded Decision

This ADR supersedes [ADR 001: Modular Versioned Documentation Structure](./001-doc-structure.md). Localization and modular documentation nodes remain valid architectural choices, but heterogeneous automatic blocks, version deltas, progressive fallback, and historical documentation selection are discontinued.
